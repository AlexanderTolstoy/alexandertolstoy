 ...
 */
public class GenerateDMCreateSQL {

    private static boolean ADD_EXCEPTION = false;
    private static boolean NEED_TYPE_CONVERSION = false;
    ...
    private static HashMap<String, ArrayList<HashMap<String,String>>> getXlsxMsg(Sheet sheet) {
        ...
        int col_num = Tools.getColNum("DM_TABLE_CREATION",COLNUM);
        boolean firstLine = true;
        int cnt = 1;
        HashMap<Integer,List<String>> configMap = ConfigCenter.config.get("DM_TABLE_CREATION");
        for(Row row : sheet){
            if(firstLine){
                firstLine = false;
                continue;
