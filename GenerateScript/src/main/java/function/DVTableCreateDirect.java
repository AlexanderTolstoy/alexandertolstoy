...
/**
 * CheckModule
 * Created by ccwei on 2019/2/21.
 */
public class DVTableCreateDirect {

    private static boolean ADD_EXCEPTION = true;

    private static boolean TEMP_PERMISSION = false;

    private static boolean HBASE_SEPERATED = true;

    private static boolean DV_FREE = true ;

    private static String tmp_str = ":key";

    private static int COLNUM = 7;
    private static HashSet<String> HBase_creation = new HashSet<String>();
    ...
    public static void loadXLSXFile(String path,String name){
        ...
        try {
            ...
            //对字段进行排序
            sortTableLine(xlsx_tables);

            //处理hbase表映射rowkey
            handleRowKey(xlsx_tables);
        ...
        //
    try{
        File sqlOutput = new File(path.replace(name,"HbaseCreation.sql"));
        OutputStream output = new FileOutputStream(sqlOutput);
        for(String str : HBase_creation)
        {
    ...
    private static void handleRowKey(HashMap<String, ArrayList<HashMap<String, String>>> xlsx_tables) {
        //处理hbase表映射rowkey
        for(String key : xlsx_tables.keySet()){
            ...
            for(HashMap<String,String> m : list){
                if(m.get(Constants.DES_IS_PRIMARY).equals(tmp_str)){
                    ++cnt;
                }
            }
            //找出所有主键
            ...
            if(cnt > 1){
                ...
                for(HashMap<String,String> m : plist){
                    ...
                    t.put(Constants.DES_IS_PRIMARY,tmp_str);
                    if(m.get(Constants.DES_IS_PRIMARY).equals(tmp_str)) {
    ...
    private static HashMap<String, HashSet<String>> getHub2SatMapping(HashMap<String, ArrayList<HashMap<String, String>>> xlsx_tables) {
        ...
        for(String key : xlsx_tables.keySet()){
            ...
            for(HashMap<String,String> line : table){
                ...
                if(colCode.startsWith("SK_H_")||colCode.startsWith("sk_h_")){
                ...
                if(colCode.startsWith("SK_L_")||colCode.startsWith("sk_l_")){
                    if(hub2key.containsKey(colCode)) {
                        System.out.println("duplicated:" + colCode);
                        warming +="duplicated:" + colCode + "\n";
                    }
        ...
        for(String key : xlsx_tables.keySet()){
            ...
            for(HashMap<String,String> line : table){
                String colCode = line.get(Constants.DES_COL_CODE);
                if((colCode.startsWith("SK_H_")||colCode.startsWith("sk_h_")||colCode.startsWith("SK_L_")||colCode.startsWith("sk_l_")) && hub2key.containsKey(colCode)){
    ...
    private static String getSubTableSQL(HashMap<String, ArrayList<HashMap<String,String>>> tables, HashMap<String, HashSet<String>> hub2Sat, String tableName) {
        ...
        res = prefix + res + sql;
        //替换schema
        res = res.replace("_schema_",getSchemaName().toUpperCase());
    ...
    private static String getSatTableAll(HashMap<String, ArrayList<HashMap<String, String>>> tables, HashMap<String, HashSet<String>> hub2Sat, String tableName, String tblCode_all, String tblCode_hb, Set<String> cfNames) {
        ...
        for(String name : hub2Sat.get(tableName)){
            ...
            for(int i = 1 ; i < table_S.size();i++){
                ...
                _attributes_ += prefix + "_" + line.get(Constants.DES_COL_CODE).toUpperCase() + "   " + line.get(Constants.DES_COL_TYPE) + "   COMMENT '" + line.get(Constants.DES_COL_COMMENT) + "'";
            }
            if (TEMP_PERMISSION) {
                //增加标记字段
                _attributes_ +=",\n";
...
//                cfName = getTblCodeOrNameOnly(line.get(Constants.DES_TABLE_CODE));
                cfName = getTblCodeOrNameSpecificPos(line.get(Constants.DES_TABLE_CODE),DV_FREE?1:2,DV_FREE?1:2);
                cfNames.add(cfName);
                _HBase_attr_ += cfName + ":" + line.get(Constants.DES_COL_CODE).toUpperCase();
            }
            if (TEMP_PERMISSION) {
                //增加一个标记字段
                _HBase_attr_ +=",\n";
        ...
        HBase_creation.add(hbCreate);

        return HBASE_SEPERATED ? res : res + hbCreate;
    }

    private static String getHbaseCreateSQL(String tblCode_hb, Set<String> cfNames) {
        String schema = getSchemaName().toUpperCase();
        String hbCreate = "create \"" + schema + ".";
        hbCreate += tblCode_hb + "\"";
    ...
    private static String handleSubTable(HashMap<String, ArrayList<HashMap<String, String>>> tables, HashMap<String, HashSet<String>> hub2Sat, String tableName, String sql, String tblCode_hb, Set<String> cfNames) {
        for(String name : hub2Sat.get(tableName)){
            ...
            for(HashMap<String, String> line : table_S){
                ...
                _attributes_ += line.get(Constants.DES_COL_CODE).toUpperCase() + "   " + line.get(Constants.DES_COL_TYPE) + "   COMMENT '" + line.get(Constants.DES_COL_COMMENT) + "'";
            }
            if (TEMP_PERMISSION) {
                //增加一个标记字段
                _attributes_ +=",\n";
...
//                }
            ...
            //过滤掉_all表
            if(line.get(Constants.DES_TABLE_CODE).contains("_all")){
                continue;
            }
            ++cnt;
            //存入hash表
    ...
    private static String getSchemaName(){
        if(ConfigCenter.config.get("DV_TABLE_CREATION") != null && ConfigCenter.config.get("DV_TABLE_CREATION").get(Constants.INDEX_SCHEMA) != null){
            return ConfigCenter.config.get("DV_TABLE_CREATION").get(Constants.INDEX_SCHEMA).get(0);
        }
        return  "";
    }
...
//        tblCode = getTblCodeOrNameOnly(tblCode);
        tblCode = getTblCodeOrNameSpecificPos(tblCode,2,2);
        return tblCode;
    }
    ...
    private static String getTblCodeOrNameSpecificPos(String input,int begin,int end){
        if(begin > end ){
            return input;
        }
        String[] strs = input.split("_");
        String res = "";
        if(strs.length < (end - begin)){
            return input;
        }
        for(int i = begin - 1;i < strs.length && i < end ; i++ ){
            if(i == begin-1){
                res+=strs[i];
            }else{
                res+= "_" + strs[i];
...
//            cfName = getTblCodeOrNameOnly(line.get(Constants.DES_TABLE_CODE));
        ...
        res = HBASE_SEPERATED ? res : hbaseTable + res;
    ...
    private static void sortTableLine(HashMap<String, ArrayList<HashMap<String,String>>> tables){

        for(String key : tables.keySet()){
            ArrayList<HashMap<String, String>> list = tables.get(key);
            for(int tail = list.size()-1; tail > 0 ; tail--){
                for(int i = 0 ; i < tail; ++i){
                    HashMap<String, String> map1 = list.get(i);
                    HashMap<String, String> map2 = list.get(i+1);
                    if(Tools.isGE(map1,map2)){
                        HashMap<String, String> tmp = new HashMap<String, String>(map1);
                        list.set(i,map2);
                        list.set(i+1,tmp);
                    }
                }
